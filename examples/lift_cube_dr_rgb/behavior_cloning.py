...
class BehaviorCloning:
    ...
    def learn(self, num_learning_iterations: int, log_dir: str) -> None:
        ...
        for it in range(num_learning_iterations):
            ...
            for batch in generator:
                ...
                top_obs_batch = batch["top_obs"]
                wrist_obs_batch = batch["wrist_obs"]
                if self.color_jitter is not None:
                    top_obs_batch = self.color_jitter(top_obs_batch)
                    wrist_obs_batch = self.color_jitter(wrist_obs_batch)

                # Forward pass for both action and pose prediction
                pred_action = self._policy(top_obs_batch, wrist_obs_batch, batch["robot_pose"])
                pred_top_pose, pred_wrist_pose = self._policy.predict_pose(top_obs_batch, wrist_obs_batch)
    ...
    def _collect_with_rl_teacher(self) -> None:
        ...
        with torch.inference_mode():
            for _ in range(self._num_steps_per_env):
                ...
                self._buffer.add(top_obs, wrist_obs, state_obs, object_pose, teacher_action)

                # Apply jitter for student action during rollout
                jittered_top_obs = top_obs.clone()
                jittered_wrist_obs = wrist_obs.clone()
                if self.color_jitter is not None:
                    jittered_top_obs = self.color_jitter(jittered_top_obs)
                    jittered_wrist_obs = self.color_jitter(jittered_wrist_obs)

                # Step environment with student action
                student_action = self._policy(jittered_top_obs.float(), jittered_wrist_obs.float(), state_obs.float())
                ...
                condition = (action_diff < 0.2).unsqueeze(-1).expand_as(student_action)
    ...
    def _evaluate_student(self, tf_writer: SummaryWriter, iteration: int) -> None:
        ...
        for _ in range(self._env.max_episode_length):
            ...
            student_action = self._policy(top_obs.float(), wrist_obs.float(), state_obs.float())
            
            obs_dict, reward, done, extras = self._env.step(student_action)
            total_reward += reward
        ...
        self._env.reset()
        self._rewbuffer.clear()
        self._cur_reward_sum.zero_()
