...
def get_train_cfg(exp_name):
    ...
    bc_cfg_dict = {
        ...
        "policy": {
            ...
            "action_head": {
                "state_obs_dim": 10,  # end-effector pose (7) + target region position (3) as state observation
...
def get_task_cfgs():
    ...
    reward_scales = {
        "reach_cube": 1.0,
        "lift_cube": 2.0,
        "reach_target": 3.0,  # ターゲットへ運ぶモチベーションを高くする
        "lower_cube": 5.0,    # 最終的に下ろす行動を最大報酬にする
        "release_cube": 10.0, # 目的領域に置いて手を離すとクリア報酬
        "action_rate": 0.01,
        "joint_vel": 0.01,
    }
...
def main():
    ...
    env_cfg["visualize_camera"] = args.vis # Link visualize_camera to -v option
    ...
    env_cfg["box_fixed"] = False  # RL trains the whole task
