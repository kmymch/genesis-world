...
class GraspEnv:
    def __init__(
        ...
        if self.num_envs <= 10:
            ...
            center_idx = self.num_envs // 2
            rendered_envs_idx = list(range(center_idx - 5, center_idx + 5))
    ...
    def _reward_keypoints(self) -> torch.Tensor:
        ...
        # 1. 実際の指先（ティップ）の位置をワールド座標系で計算する
        # wxai_follower の指先(ee_site)は center_finger_pose(キャリッジ原点) からローカルの +X 方向に正確に 6.95cm(0.0695m) の位置にあります
        local_tip_offset = torch.tensor([[0.0695, 0.0, 0.0]], device=self.device, dtype=gs.tc_float).repeat(self.num_envs, 1, 1)
        finger_pos = self.robot.center_finger_pose[:, :3].unsqueeze(1)
        finger_quat = self.robot.center_finger_pose[:, 3:7].unsqueeze(1)
        tip_pos_world = transform_by_trans_quat(local_tip_offset, finger_pos, finger_quat).squeeze(1)
        
        # 2. 姿勢マッチングのための目標姿勢オフセット
        # wxai_follower はローカル +X 方向に把持しますが、環境のデフォルトのターゲット姿勢(q_downward)は -Z 方向を向いています
        # 手先のクォータニオンを Y軸回りに -90度 回転させるオフセットを適用し、+X が -Z と一致するように補正します
        q_offset = torch.tensor([0.7071068, 0.0, -0.7071068, 0.0], device=self.device, dtype=gs.tc_float).expand(self.num_envs, -1)
        finger_grasp_quat = transform_quat_by_quat(self.robot.center_finger_pose[:, 3:7], q_offset)
        
        # 3. 補正した位置と姿勢でキーポイントをワールド座標に変換して距離を計算
    ...
    def _reward_lift(self) -> torch.Tensor:
        # object z coordinate
        obj_z = self.object.get_pos()[:, 2]
        # Box rests at Z=0.025. If lifted > 0.035, give reward. Max reward at 0.085.
        lift_height = obj_z - 0.035
        reward = torch.clamp(lift_height * 20.0, min=0.0, max=1.0)
        return reward

    def _reward_gripper_open(self) -> torch.Tensor:
        # Get the current gripper position
        q_pos = self.robot._robot_entity.get_qpos()
        left_finger = q_pos[:, self.robot._left_finger_dof]
        right_finger = q_pos[:, self.robot._right_finger_dof]
        gripper_pos = (left_finger + right_finger) / 2.0
        
        # Normalize between 0 (fully closed) and 1 (fully open)
        ...
        return torch.clamp(normalized_open, min=0.0, max=1.0)
